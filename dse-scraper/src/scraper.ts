...
async function saveToDatabase(data: CompanyInfo[]): Promise<void> {
    try {
        ...
        console.log(`📊 Database save summary: ${successCount} success, ${errorCount} errors`);
    } catch (error) {
...
async function scrapeAllCompanies(): Promise<void> {
    ...
    if (!isApiHealthy) {
        console.error("❌ API is not available. Please make sure the DSE API server is running.");
        console.log("💡 Start the API server with: cd dse-api && bun run start:dev");
        return;
    }
