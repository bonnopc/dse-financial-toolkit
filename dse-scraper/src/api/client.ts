...
export class DSEApiClient {
    ...
    async createCompany(companyData: CompanyInfo): Promise<any> {
        try {
            ...
            const response = await axios.put(`${this.baseURL}/companies/upsert`, apiData, {
                headers: {
            ...
            if (axios.isAxiosError(error)) {
                if (error.response?.status === 400) {
                    ...
                    console.error(`❌ Data validation error for ${companyData.name}:`, error.response?.data?.message || 'Invalid data format');
                    return null;
                } else {
                    console.error(`❌ API Error for ${companyData.name} (${error.response?.status}):`, error.response?.data?.message || error.message);
                    return null;
                }
            } else {
    ...
    private transformCompanyData(companyData: CompanyInfo): any {
        ...
        const safeDividends = Array.isArray(companyData.dividends) ? companyData.dividends.map(dividend => ({
        ...
        const safeFinancialPerformance = Array.isArray(companyData.financialPerformance) ? 
        ...
        return {
            ...
            loans: {
                ...
                dateUpdated: companyData.loans?.dateUpdated || new Date().toISOString().split('T')[0],
            },
            reserveAndIncome: {
                reserveMillion: cleanNumeric(companyData.reserveAndIncome?.reserveInMillion),
                unappropriatedProfitMillion: cleanNumeric(companyData.reserveAndIncome?.ociInMillion),
                dateUpdated: new Date().toISOString().split('T')[0], // Use current date as fallback
            },
            metadata: {
                sector: companyData.metadata?.sector || 'Unknown',
                authorizedCapitalInMillion: cleanNumeric(companyData.metadata?.authorizedCapitalInMillion),
                paidUpCapitalInMillion: cleanNumeric(companyData.metadata?.paidUpCapitalInMillion),
                shareCount: cleanNumeric(companyData.metadata?.shareCount),
            },
            priceInfo: {
                lastTradingPrice: cleanNumeric(companyData.priceInfo?.lastTradingPrice) || 0,
                ...
                week52Min: cleanNumeric(companyData.priceInfo?.movingRangeFor52Weeks?.min) || 0,
                week52Max: cleanNumeric(companyData.priceInfo?.movingRangeFor52Weeks?.max) || 0,
            },
            ...
            otherInfo: {
                ...
                shareHoldingParcentages: companyData.otherInfo?.shareHoldingParcentages || [],
            },
