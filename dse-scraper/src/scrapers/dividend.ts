...
function getLoanData($: cheerio.Root, loanTable: cheerio.Cheerio): CompanyLoan {
  ...
  loanTable.find('tbody tr').each((_index, row) => {
    ...
    if (
      cols.length > 2 &&
      $(cols[1]).text().trim().toLowerCase().includes('short-term loan (mn)')
    ) {
    ...
    if (
      cols.length > 1 &&
      $(cols[0]).text().trim().includes('Long-term loan (mn)')
    ) {
    ...
    if (
      cols.length === 1 &&
      $(cols[0]).text().trim().includes('Present Loan Status as')
    ) {
...
function getReserveAndIncome(
  $: cheerio.Root,
  reserveAndIncomeTable: cheerio.Cheerio
): ReserveAndIncome {
  ...
  reserveAndIncomeTable.find('tbody tr').each((_index, row) => {
    ...
    if (
      ...
      $(cols[0])
        ...
        .trim()
        .toLowerCase()
        .includes('reserve & surplus without oci (mn)')
    ) {
      const values = $(row).find('td');
      reserveInMillion = parseFloat(
        $(values[0]).text().trim().replace(',', '')
      );
    }
    ...
    if (
      ...
      $(cols[0])
        ...
        .trim()
        .toLowerCase()
        .includes('other comprehensive income (oci) (mn)')
    ) {
      const values = $(row).find('td');
...
function getMetadata(
  ...
  metadataTable.find('tbody tr').each((_index, row) => {
    ...
    if (
      cols.length > 1 &&
      $(cols[1]).text().trim().toLowerCase().includes('sector')
    ) {
      const values = $(row).find('td');
    ...
    if (
      cols.length > 1 &&
      $(cols[0]).text().trim().toLowerCase().includes('authorized capital (mn)')
    ) {
      const values = $(row).find('td');
    ...
    if (
      cols.length > 1 &&
      $(cols[0]).text().trim().toLowerCase().includes('paid-up capital (mn)')
    ) {
      const values = $(row).find('td');
    ...
    if (
      ...
      $(cols[0])
        ...
        .trim()
        .toLowerCase()
        .includes('total no. of outstanding securities')
    ) {
      const values = $(row).find('td');
...
function getPriceInfo(
  $: cheerio.Root,
  priceInfoTable: cheerio.Cheerio
): PriceInfo {
  ...
  priceInfoTable.find('tbody tr').each((_index, row) => {
    ...
    if (
      headers.length > 1 &&
      $(headers[0]).text().trim().toLowerCase().includes('last trading price')
    ) {
    ...
    if (
      headers.length > 0 &&
      $(headers[0]).text().trim().toLowerCase().includes('52 weeks')
    ) {
      const rangeText = $(cols[1]).text().trim();
      const rangeParts = rangeText.split('-');
      if (rangeParts.length === 2) {
        movingRangeFor52Weeks.min = parseFloat(
          rangeParts[0].trim().replace(/,/g, '')
        );
        movingRangeFor52Weeks.max = parseFloat(
          rangeParts[1].trim().replace(/,/g, '')
        );
      }
    }
  });
...
function getFinancialPerformance(
  $: cheerio.Root,
  financialPerformanceTable: cheerio.Cheerio
): FinancialPerformance[] {
  ...
  financialPerformanceTable.find('tbody tr').each((_index, row) => {
    ...
    if (cols.length > 12) {
      ...
      if (!isNaN(year)) {
        const profitInMillion = parseFloat(
          $(cols[11]).text().trim().replace(/,/g, '')
        );
        const originalEarningsPerShare = parseFloat(
          $(cols[4]).text().trim().replace(/,/g, '')
        );
        const restatedEarningsPerShare = parseFloat(
          $(cols[5]).text().trim().replace(/,/g, '')
        );
        const dilutedEarningsPerShare = parseFloat(
          $(cols[6]).text().trim().replace(/,/g, '')
        );
        const originalNAVPerShare = parseFloat(
          $(cols[7]).text().trim().replace(/,/g, '')
        );
        const restatedNAVPerShare = parseFloat(
          $(cols[8]).text().trim().replace(/,/g, '')
        );
        const dilutedNAVPerShare = parseFloat(
          $(cols[9]).text().trim().replace(/,/g, '')
        );
        const earningsPerShare = isNaN(dilutedEarningsPerShare)
          ? isNaN(restatedEarningsPerShare)
            ? originalEarningsPerShare
            : restatedEarningsPerShare
          : dilutedEarningsPerShare;
        const navPerShare = isNaN(dilutedNAVPerShare)
          ? isNaN(restatedNAVPerShare)
            ? originalNAVPerShare
            : restatedNAVPerShare
          : dilutedNAVPerShare;
...
function getOtherInfo(
  $: cheerio.Root,
  otherInfoTable: cheerio.Cheerio
): OtherInfo {
  let listingYear = 0;
  let marketCategory = '';
  ...
  otherInfoTable.find('tbody tr').each((_index, row) => {
    ...
    if (
      cols.length > 1 &&
      $(cols[0]).text().trim().toLowerCase().includes('listing year')
    ) {
    ...
    if (
      cols.length > 1 &&
      $(cols[0]).text().trim().toLowerCase().includes('market category')
    ) {
    ...
    if (
      ...
      $(cols[0])
        ...
        .trim()
        .toLowerCase()
        .includes('share holding percentage')
    ) {
...
function getUnauditedPERatio(
  $: cheerio.Root,
  peRatioTable: cheerio.Cheerio
): number {
  ...
  peRatioTable.find('tbody tr').each((_index, row) => {
    ...
    if (
      ...
      $(cols[0])
        ...
        .trim()
        .toLowerCase()
        .includes('current p/e ratio using diluted eps')
    ) {
    ...
    if (
      ...
      $(cols[0])
        ...
        .trim()
        .toLowerCase()
        .includes('current p/e ratio using basic eps')
    ) {
      basicEpsPeRatio = parseFloat($(cols[6]).text().trim());
    }
  });

  const unauditedPERatio =
    !dilutedEpsPeRatio || isNaN(dilutedEpsPeRatio)
      ? isNaN(basicEpsPeRatio)
        ? 0
        : basicEpsPeRatio
      : dilutedEpsPeRatio;
...
export function getCompanyData($: cheerio.Root): CompanyData {
  ...
  const reserveAndIncome = getReserveAndIncome(
    $,
    tablesToFind.reserveAndIncomeTable.table
  );
  const metadata = getMetadata($, tablesToFind.metadata.table);
  const priceInfo = getPriceInfo($, tablesToFind.priceInfo.table);
  const financialPerformance = getFinancialPerformance(
    $,
    tablesToFind.financialPerformance.table
  );
