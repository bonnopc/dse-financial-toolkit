...
async function fetchDividendData(companyCode: string) {
    try {
        const response = await axios.get(`https://www.dsebd.org/displayCompany.php?name=${companyCode}`, {
            timeout: 10000
        });
        const $ = cheerio.load(response.data);
        return getCompanyData($);
    } catch (error) {
        console.error(`Error fetching data for ${companyCode}:`, error instanceof Error ? error.message : error);
        return null;
    }
}
...
async function testSpecificCompanies() {
    ...
    for (const company of testCompanies) {
        try {
            ...
            // Extract full name
            const response = await axios.get(`https://www.dsebd.org/displayCompany.php?name=${company}`);
            const $ = cheerio.load(response.data);
            ...
            try {
                ...
                console.error(`❌ ${company}: API Error -`, apiError instanceof Error ? apiError.message : apiError);
            }

        } catch (error) {
            console.error(`❌ ${company}: Scraping Error -`, error instanceof Error ? error.message : error);
        }
    }
}

testSpecificCompanies().then(() => {
    console.log("\n🧪 Test completed!");
    process.exit(0);
}).catch(error => {
