import { Company, FinancialPerformance } from '../types/Company';
...
export const calculateFinancialScore = (
  ...
  const paidUpCapital = company.metadata.paidUpCapitalInMillion || 1; // Avoid division by zero
  const shortTermLoan = company.loans.shortTermMillion || 0;
  const longTermLoan = company.loans.longTermMillion || 0;
  const reserveAmount = company.reserveAndIncome.reserveInMillion || 0;
  ...
  if (dividends.length === 0) {
    ...
    if (dividends.length >= 3) {
      ...
      if (older3Years.length > 0) {
        ...
        if (recentAvg > olderAvg * 1.1)
          trendScore = 100; // Improving trend
        else if (recentAvg >= olderAvg * 0.9)
          trendScore = 80; // Stable trend
    ...
    if (dividends.length >= 4) {
      ...
      if (consecutiveDividends) {
        dividendConsistencyScore = Math.min(
          100,
          dividendConsistencyScore * 1.1
        ); // 10% bonus
      }
    }
  }
  ...
  if (
    ...
    company.priceInfo.lastTradingPrice > 0
  ) {
    ...
    const dividendYield =
      (latestCashDividendAmount / company.priceInfo.lastTradingPrice) * 100;

    if (dividendYield >= 8)
      dividendYieldScore = 100; // Excellent yield: 8%+
    else if (dividendYield >= 5)
      dividendYieldScore = 80; // Good yield: 5-8%
    else if (dividendYield >= 3)
      dividendYieldScore = 60; // Average yield: 3-5%
    else if (dividendYield >= 1)
      dividendYieldScore = 40; // Poor yield: 1-3%
    else if (dividendYield > 0)
      dividendYieldScore = 20; // Very poor yield: <1%
  ...
  if (
    ...
    if (max52Week > min52Week) {
      // Valid range
      const pricePosition =
        ((currentPrice - min52Week) / (max52Week - min52Week)) * 100;

      if (pricePosition <= 25)
        priceValuationScore = 100; // Near 52-week low - excellent buying opportunity
      else if (pricePosition <= 40)
        priceValuationScore = 80; // Below average - good value
      else if (pricePosition <= 60)
        priceValuationScore = 60; // Average range - fair value
      else if (pricePosition <= 75)
        priceValuationScore = 40; // Above average - consider carefully
  ...
  if (company.financialPerformance && company.financialPerformance.length > 0) {
    const performances = company.financialPerformance.sort(
      (a: FinancialPerformance, b: FinancialPerformance) => a.year - b.year
    );

    // Factor 1: Consistency - How many years with positive EPS (60% weight)
    const positiveYears = performances.filter(
      (p: FinancialPerformance) => p.earningsPerShare > 0
    ).length;
    const consistencyRatio = positiveYears / performances.length;
    ...
    if (performances.length >= 3) {
      const recent3Years = performances.slice(-3);
      const older3Years = performances.slice(-6, -3);

      if (older3Years.length > 0) {
        const recentAvgEPS =
          recent3Years.reduce(
            (sum: number, p: FinancialPerformance) => sum + p.earningsPerShare,
            0
          ) / recent3Years.length;
        const olderAvgEPS =
          older3Years.reduce(
        ...
        if (recentAvgEPS > olderAvgEPS * 1.2)
          growthScore = 100; // Strong EPS growth
        else if (recentAvgEPS > olderAvgEPS * 1.1)
          growthScore = 80; // Good EPS growth
        else if (recentAvgEPS >= olderAvgEPS * 0.9)
          growthScore = 60; // Stable EPS
        else if (recentAvgEPS >= olderAvgEPS * 0.7)
          growthScore = 40; // Declining EPS
  ...
  let shareholdingQualityScore = 0; // Start with 0, build incrementally
  if (
    company.otherInfo?.shareHoldingParcentages &&
    company.otherInfo.shareHoldingParcentages.length > 0
  ) {
    const latestShares = company.otherInfo.shareHoldingParcentages.sort(
    ...
    // 0% = 0pts, 10% = 20pts, 25% = 50pts, 50% = 100pts, 75%+ = 100pts
    ...
    // 0% = 0pts, 5% = 25pts, 15% = 50pts, 30% = 75pts, 50%+ = 100pts
    ...
    // 0% = 0pts, 2% = 20pts, 5% = 40pts, 10% = 60pts, 20% = 80pts, 30%+ = 100pts
  ...
  if (company.unauditedPERatio && company.unauditedPERatio > 0) {
    const pe = company.unauditedPERatio;

    if (pe <= 8)
      peRatioScore = 100; // Excellent value
    else if (pe <= 12)
      peRatioScore = 90; // Very good value
    else if (pe <= 15)
      peRatioScore = 80; // Good value
    else if (pe <= 18)
      peRatioScore = 70; // Fair value
    else if (pe <= 22)
      peRatioScore = 60; // Average value
    else if (pe <= 28)
      peRatioScore = 40; // Poor value
    else if (pe <= 35)
      peRatioScore = 20; // Very poor value
    else peRatioScore = 10; // Extremely overvalued
  }

  // Calculate overall score (weighted average with 8 components - optimized weights for DSE context)
  const overall = Math.round(
    debtScore * 0.1 + // Debt ratio - 10% (financial risk management)
      reserveScore * 0.1 + // Financial reserves - 10% (balance sheet strength)
      dividendConsistencyScore * 0.15 + // Dividend consistency - 15% (long-term reliability)
      dividendYieldScore * 0.1 + // Dividend yield - 10% (reward good yield, avoid traps)
      priceValuationScore * 0.1 + // Price valuation - 10% (position in 52-week range)
      peRatioScore * 0.1 + // PE ratio - 10% (earnings-based valuation)
      financialPerformanceScore * 0.25 + // EPS performance - 25% (highest weight: profitability & growth)
      shareholdingQualityScore * 0.1 // Shareholding quality - 10% (governance & confidence)
  );
