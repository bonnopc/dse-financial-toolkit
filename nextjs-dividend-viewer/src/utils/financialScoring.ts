...
export const calculateFinancialScore = (company: Company, allCompanies: Company[]): FinancialScore => {
  ...
  if (dividends.length === 0) {
    ...
    const nonZeroDividends = dividends.filter(d => getWeightedDividend(d.cashDividend, d.stockDividend) > 0);
    ...
    const totalDividends = dividends.map(d => getWeightedDividend(d.cashDividend, d.stockDividend));
    ...
    if (dividends.length >= 3) {
      const recent3Years = dividends.slice(-3).map(d => getWeightedDividend(d.cashDividend, d.stockDividend));
      const older3Years = dividends.slice(-6, -3).map(d => getWeightedDividend(d.cashDividend, d.stockDividend));
    ...
    if (dividends.length >= 4) {
      ...
      const consecutiveDividends = lastFourYears.every(d => getWeightedDividend(d.cashDividend, d.stockDividend) > 0);
  ...
  if (dividends.length > 0 && company.priceInfo?.lastTradingPrice && company.priceInfo.lastTradingPrice > 0) {
    ...
    const dividendYield = (latestCashDividendAmount / company.priceInfo.lastTradingPrice) * 100;
  ...
  if (company.financialPerformance && company.financialPerformance.length > 0) {
    const performances = company.financialPerformance.sort((a: FinancialPerformance, b: FinancialPerformance) => a.year - b.year);
    
    // Factor 1: Consistency - How many years with positive EPS (60% weight)
    const positiveYears = performances.filter((p: FinancialPerformance) => p.earningsPerShare > 0).length;
    ...
    if (performances.length >= 3) {
      ...
      if (older3Years.length > 0) {
        const recentAvgEPS = recent3Years.reduce((sum: number, p: FinancialPerformance) => sum + p.earningsPerShare, 0) / recent3Years.length;
        const olderAvgEPS = older3Years.reduce((sum: number, p: FinancialPerformance) => sum + p.earningsPerShare, 0) / older3Years.length;
  ...
  if (company.otherInfo?.shareHoldingParcentages && company.otherInfo.shareHoldingParcentages.length > 0) {
    const latestShares = company.otherInfo.shareHoldingParcentages
  ...
  const overall = Math.round(
    (debtScore * 0.10) +                      // Debt ratio - 10% (financial risk management)
    (reserveScore * 0.10) +                   // Financial reserves - 10% (balance sheet strength)
    (dividendConsistencyScore * 0.15) +       // Dividend consistency - 15% (long-term reliability)
    (dividendYieldScore * 0.10) +             // Dividend yield - 10% (reward good yield, avoid traps)
    (priceValuationScore * 0.10) +            // Price valuation - 10% (position in 52-week range)
    (peRatioScore * 0.10) +                   // PE ratio - 10% (earnings-based valuation)
    (financialPerformanceScore * 0.25) +      // EPS performance - 25% (highest weight: profitability & growth)
    (shareholdingQualityScore * 0.10)         // Shareholding quality - 10% (governance & confidence)
  );
