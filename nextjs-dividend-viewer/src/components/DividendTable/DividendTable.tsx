...
const DividendTable: React.FC<DividendTableProps> = ({ companies }) => {
  ...
  const getTotalDividends = (company: Company) => {
    return company.dividends.reduce((total, dividend) => 
      total + dividend.cashDividend + dividend.stockDividend, 0
    );
  };
  ...
  const getLatestFinancialPerformance = (company: Company) => {
    ...
    const sortedPerformance = [...company.financialPerformance].sort((a, b) => b.year - a.year);
  ...
  const getFinancialPerformanceChange = (company: Company, field: 'profitInMillion' | 'earningsPerShare' | 'NAVPerShare') => {
    ...
    const sortedPerformances = [...performances].sort((a, b) => b.year - a.year);
  ...
  const FinancialPerformanceCell: React.FC<{ 
    ...
    const changeClass = data.changePercent > 0 ? 'financial-increase' : 
                       data.changePercent < 0 ? 'financial-decrease' : 
                       'financial-neutral';
  ...
  const getCurrentDividendYield = (company: Company) => {
    ...
    const sortedDividends = [...company.dividends].sort((a, b) => b.year - a.year);
  ...
  const getLatestShareholding = (company: Company) => {
    ...
    const sortedShares = [...company.otherInfo.shareHoldingParcentages].sort((a, b) => {
  ...
  const ShareholdingCell: React.FC<{ 
    company: Company, 
    field: 'sponsorOrDirector' | 'government' | 'institution' | 'foreign' | 'publicShares' 
  }> = ({ company, field }) => {
    ...
    const changeClass = data.changePercent > 0 ? 'shareholding-increase' : 
                       data.changePercent < 0 ? 'shareholding-decrease' : 
      ...
      <div className="table-scroll-wrapper">
        <table className="dividend-table">
          <thead>
            <tr>
              ...
              <SortableHeader field="latestProfit">Latest Profit (M)</SortableHeader>
              ...
              <SortableHeader field="cashDividend">Latest Cash Dividend (%)</SortableHeader>
              <SortableHeader field="stockDividend">Latest Stock Dividend (%)</SortableHeader>
              <SortableHeader field="totalDividends">Total Historical Dividends</SortableHeader>
              <SortableHeader field="shortTermLoan">Short Term Loan (M)</SortableHeader>
              <SortableHeader field="longTermLoan">Long Term Loan (M)</SortableHeader>
        ...
        <tbody>
          {sortedCompanies.map((company, index) => {
            ...
            return (
              <tr key={company.name} className={index % 2 === 0 ? 'even-row' : 'odd-row'}>
                ...
                <td className={`price-position ${getPricePositionClass(getPricePosition(company))}`}>
                  <div className="price-position-container">
                    <div className="price-position-bar">
                      <div 
                        className="price-position-indicator" 
                        style={{left: `${Math.min(100, Math.max(0, getPricePosition(company)))}%`}}
                      ></div>
                    </div>
                    <span className="price-position-text">
                      {getPricePosition(company) > 0 ? `${getPricePosition(company).toFixed(1)}%` : 'N/A'}
                    </span>
                  </div>
                </td>
                <td className={`dividend-yield-value ${(() => {
                ...
                <td className={`pe-ratio-value ${(() => {
                  ...
                  {company.unauditedPERatio ? company.unauditedPERatio.toFixed(2) : 'N/A'}
                </td>
                <td className="financial-performance-value">
                  <FinancialPerformanceCell company={company} field="profitInMillion" suffix="M" />
                </td>
                <td className="financial-performance-value">
                  <FinancialPerformanceCell company={company} field="earningsPerShare" suffix="৳" />
                </td>
                <td className="financial-performance-value">
                  <FinancialPerformanceCell company={company} field="NAVPerShare" suffix="৳" />
                </td>
                <td>{latestDividend.year || 'N/A'}</td>
                <td className="dividend-value">{latestDividend.cashDividend}%</td>
                <td className="dividend-value">{latestDividend.stockDividend}%</td>
                ...
                <td className="loan-value">{company.loans.shortTermMillion.toLocaleString()}</td>
                <td className="loan-value">{company.loans.longTermMillion.toLocaleString()}</td>
                <td className="loan-value total-loan">{totalLoan.toLocaleString()}</td>
                <td className="reserve-value">{company.reserveAndIncome.reserveInMillion.toLocaleString()}</td>
                <td className="capital">{company.metadata.paidUpCapitalInMillion.toLocaleString()}</td>
                <td className="share-count">{company.metadata.shareCount ? company.metadata.shareCount.toLocaleString() : 'N/A'}</td>
                <td className="shareholding-value">
                  <ShareholdingCell company={company} field="sponsorOrDirector" />
                </td>
                <td className="shareholding-value">
