import { Company, Dividend, FinancialPerformance, ShareHolding } from '@/types/Company';
import { ApiCompany, ApiDividend, ApiFinancialPerformance, ApiShareholdingPercentage } from './dse-api-client';
...
export function transformApiCompanyToCompany(apiCompany: ApiCompany): Company {
  return {
    ...
    dividends: apiCompany.dividends ? transformApiDividends(apiCompany.dividends) : [],
    
    loans: {
      shortTermMillion: parseFloat(apiCompany.short_term_million?.toString() || '0'),
      longTermMillion: parseFloat(apiCompany.long_term_million?.toString() || '0'),
      dateUpdated: apiCompany.date_updated || new Date().toISOString().split('T')[0],
    },
    
    reserveAndIncome: {
      reserveInMillion: parseFloat(apiCompany.reserve_million?.toString() || '0'),
      ociInMillion: parseFloat(apiCompany.unappropriated_profit_million?.toString() || '0'),
    },
    
    metadata: {
      sector: apiCompany.sector || 'Unknown',
      authorizedCapitalInMillion: parseFloat(apiCompany.authorized_capital_million?.toString() || '0'),
      paidUpCapitalInMillion: parseFloat(apiCompany.paid_up_capital_million?.toString() || '0'),
      shareCount: parseInt(apiCompany.share_count?.toString() || '0') || undefined,
    },
    ...
    otherInfo: {
      ...
      shareHoldingParcentages: apiCompany.shareholding_percentages ? 
        transformApiShareholdingPercentages(apiCompany.shareholding_percentages) : [],
    },
    ...
    unauditedPERatio: apiCompany.unaudited_pe_ratio ? parseFloat(apiCompany.unaudited_pe_ratio) : undefined,
  };
}
 ...
 */
export function transformApiFinancialPerformance(apiPerformance: ApiFinancialPerformance[]): FinancialPerformance[] {
 ...
 */
export function transformApiShareholdingPercentages(apiShareholding: ApiShareholdingPercentage[]): ShareHolding[] {
  return apiShareholding.map(holding => ({
    date: holding.date_recorded,
    sponsorOrDirector: parseFloat(holding.sponsor_or_director?.toString() || '0'),
 ...
 */
export function transformApiCompaniesToCompanies(apiCompanies: ApiCompany[]): Company[] {
