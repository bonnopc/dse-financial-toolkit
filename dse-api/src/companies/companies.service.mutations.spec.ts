...
describe('CompaniesService - createCompany and upsertCompany', () => {
  ...
  describe('createCompany', () => {
    ...
    it('should create a new company successfully', async () => {
      ...
      // Check all properties except timestamps which may vary by milliseconds
      expect(result.code).toEqual(mockCompany.code);
      expect(result.full_name).toEqual(mockCompany.full_name);
      expect(result.financial_score).toEqual(mockCompany.financial_score);
      expect(result.financial_score_components).toEqual(
        mockCompany.financial_score_components
      );
      expect(result.id).toEqual(mockCompany.id);
      expect(result.sector).toEqual(mockCompany.sector);
      expect(result.listing_year).toEqual(mockCompany.listing_year);
      expect(result.market_category).toEqual(mockCompany.market_category);
      expect(result.unaudited_pe_ratio).toEqual(mockCompany.unaudited_pe_ratio);
      // Check that timestamps are dates and reasonable (within last few seconds)
    ...
    it('should insert dividends data', async () => {
      const mockTrx = DatabaseTestUtils.setupCompanyCreationMocks(
        mockDb,
        'success'
      );
    ...
    it('should insert financial performance data', async () => {
      const mockTrx = DatabaseTestUtils.setupCompanyCreationMocks(
        mockDb,
        'success'
      );
