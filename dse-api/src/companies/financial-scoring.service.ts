...
export class FinancialScoringService {
   ...
   */
  calculateFinancialScore(companyData: CompanyFinancialData): FinancialScoreResult {
    ...
    let dividendConsistencyScore = this.calculateDividendConsistencyScore(dividends);

    // 4. Dividend Yield Score (10% weight)
    let dividendYieldScore = this.calculateDividendYieldScore(dividends, companyData.lastTradingPrice);
    ...
    let financialPerformanceScore = this.calculateFinancialPerformanceScore(companyData.financialPerformance);

    // 7. Shareholding Quality Score (10% weight)
    let shareholdingQualityScore = this.calculateShareholdingQualityScore(companyData.shareholdingPercentages);
    ...
    const overall = Math.round(
      ...
      (dividendYieldScore * 0.10) +
      (priceValuationScore * 0.10) +
      (peRatioScore * 0.10) +
      (financialPerformanceScore * 0.25) +
      (shareholdingQualityScore * 0.10)
    );
  ...
  private calculateDividendConsistencyScore(dividends: Array<{ year: number; cashDividend: number; stockDividend: number }>): number {
    ...
    let consistencyScore = (frequencyScore * 0.4) + (stabilityScore * 0.3) + (trendScore * 0.3);
  ...
  private calculateDividendYieldScore(dividends: Array<{ year: number; cashDividend: number; stockDividend: number }>, lastTradingPrice?: number): number {
  ...
  private calculatePriceValuationScore(lastTradingPrice?: number, movingRange?: { min: number; max: number }): number {
    if (!lastTradingPrice || !movingRange || movingRange.max <= movingRange.min) {
      return 50; // Default neutral score
    }

    const pricePosition = ((lastTradingPrice - movingRange.min) / (movingRange.max - movingRange.min)) * 100;
  ...
  private calculateFinancialPerformanceScore(performances: Array<{ year: number; earningsPerShare: number }>): number {
    ...
    const positiveYears = sortedPerformances.filter(p => p.earningsPerShare > 0).length;
    ...
    if (sortedPerformances.length >= 3) {
      ...
      if (older3Years.length > 0) {
        const recentAvgEPS = recent3Years.reduce((sum, p) => sum + p.earningsPerShare, 0) / recent3Years.length;
        const olderAvgEPS = older3Years.reduce((sum, p) => sum + p.earningsPerShare, 0) / older3Years.length;
  ...
  private calculateShareholdingQualityScore(shareholdings: Array<{ date: string; sponsorOrDirector: number; institution: number; foreign: number }>): number {
    ...
    return (sponsorScore * sponsorWeight) + 
