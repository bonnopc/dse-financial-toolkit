...
} from '../types/financial-score.types';
  ...
  calculateFinancialScore(company: CompanyWithScore): FinancialScore {
    ...
    if (dividends.length === 0) {
      ...
      const nonZeroDividends = dividends.filter(
        (d) => getWeightedDividend(d.cash_dividend, d.stock_dividend) > 0
      );
      ...
      const totalDividends = dividends.map((d) =>
        getWeightedDividend(d.cash_dividend, d.stock_dividend)
      );
      const avgDividend =
        totalDividends.reduce((sum, div) => sum + div, 0) /
      ...
      if (dividends.length >= 3) {
        const recent3Years = dividends
          .slice(-3)
          .map((d) => getWeightedDividend(d.cash_dividend, d.stock_dividend));
        ...
        if (older3Years.length > 0) {
          const recentAvg =
            recent3Years.reduce((sum, div) => sum + div, 0) /
            recent3Years.length;
          const olderAvg =
    ...
    if (
      dividends.length > 0 &&
      company.last_trading_price &&
      company.last_trading_price > 0
    ) {
      ...
      const latestCashDividendAmount =
        (latestCashDividendPercentage / 100) * 10; // Face value is 10 taka

      // Calculate actual dividend yield as percentage (dividend amount in taka / share price * 100)
      const dividendYield =
        (latestCashDividendAmount / company.last_trading_price) * 100;
    ...
    if (performances.length > 0) {
      ...
      if (sortedPerformances.length >= 3) {
        ...
        if (older3Years.length > 0) {
          const recentAvgEPS =
            recent3Years.reduce((sum, p) => sum + p.earnings_per_share, 0) /
            recent3Years.length;
          const olderAvgEPS =
            older3Years.reduce((sum, p) => sum + p.earnings_per_share, 0) /
            older3Years.length;
    ...
    const overall = Math.round(
      debtScore * 0.1 + // Debt ratio - 10%
        reserveScore * 0.1 + // Financial reserves - 10%
        dividendConsistencyScore * 0.15 + // Dividend consistency - 15%
        dividendYieldScore * 0.1 + // Dividend yield - 10%
        priceValuationScore * 0.1 + // Price valuation - 10%
        peRatioScore * 0.1 + // PE ratio - 10%
        financialPerformanceScore * 0.25 + // EPS performance - 25%
        shareholdingQualityScore * 0.1 // Shareholding quality - 10%
    );
